...
const(
	...
	cloud_in_url="https://api.eeo.cn/partner/api/cloud.api.php?action="
)
...
func (C *Classin)RPC_cloud (action string,formdata map[string]interface{})(*simplejson.Json, error){
	client := &http.Client{
		Jar: jar,
		Timeout:3*time.Second,
	}
	...
	log.Info(writer)
	req, err := http.NewRequest("POST", cloud_in_url+action, body)
	if err != nil {
		log.Error(err)
		return nil, errors.New("New request error")
	}
