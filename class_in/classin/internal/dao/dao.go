...
func CreateTable(db *gorm.DB) {
	if !db.HasTable(model.IdentificationRecord{}) {
		db.CreateTable(&model.IdentificationRecord{})
		log4go.Info("创建相机表成功")
	} else {
		db.AutoMigrate(&model.IdentificationRecord{})
		log4go.Info("更新相机表成功")
	}
	if !db.HasTable(model.Stranger{}) {
		db.CreateTable(&model.Stranger{})
		log4go.Info("创建相机表成功")
	} else {
		db.AutoMigrate(&model.Stranger{})
		log4go.Info("更新相机表成功")
	}
	if !db.HasTable(model.Statistic{}) {
		db.CreateTable(&model.Statistic{})
		log4go.Info("创建相机表成功")
	} else {
		db.AutoMigrate(&model.Statistic{})
		log4go.Info("更新相机表成功")
	}
	if !db.HasTable(model.Statistic_Main{}) {
		db.CreateTable(&model.Statistic_Main{})
		log4go.Info("创建统计项表成功")
	} else {
		db.AutoMigrate(&model.Statistic_Main{})
		log4go.Info("更新统计项表成功")
	}
	if !db.HasTable(model.Statistic_Item{}) {
		db.CreateTable(&model.Statistic_Item{})
		log4go.Info("创建统计小项表成功")
	} else {
		db.AutoMigrate(&model.Statistic_Item{})
		log4go.Info("更新统计小项表成功")
	}
	if !db.HasTable(model.Statistics{}) {
		db.CreateTable(&model.Statistics{})
		log4go.Info("创建Statistics表成功")
	} else {
		db.AutoMigrate(&model.Statistics{})
		log4go.Info("更新Statistics表成功")
	}
